      ...
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <!-- 属性值 -->
            <li class="with-x" v-for="(attrValue,index) in searchParams.props" :key="attrValue">
            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOrder('1')">
                  <a>综合
                    <span v-show="isOne">
                      <span v-show="isAsc">⬆</span><span v-show="isDesc">⬇</span>
                    </span>
                  </a>
                </li>
                <!-- <li>
...
export default {
  ...
  //mounted只会执行一次，将获取数据放在methods中能够多次进行数据请求
  beforeMount() {
    //在发请求之前，将所需要请求的数据进行合并
  ...
  watch: {
    $route(newValue, oldValue) {
      // console.log(11,this.searchParams)
      //在beforeMount只会执行一次，再次触发请求不会再触发，需要进行监听搜索时再次触发
      Object.assign(this.searchParams, this.$route.query, this.$route.params);
      this.getData();
