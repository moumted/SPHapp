      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart,index) in cartInfoList" :key="cart.id">
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked="cart.isChecked==1" @change="updateChange(cart,$event)">
          </li>
  ...
  export default {
    ...
    methods : {
      ...
      async updateAllCartChecked(event){
        try {
          let isChecked = event.target.checked?"1":"0"
          await this.$store.dispatch('upDateCartChecked',isChecked)
          this.getDate()
        } catch (error) {
          alert(error.message)
        }
